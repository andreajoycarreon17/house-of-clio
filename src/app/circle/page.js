  ...
  return (
    <>
      ...
      <section style={{ background: T.bg2, padding: "0 clamp(40px,6vw,80px)" }}>
        <Mx w={960}><Rv><RealImg src={IMG.candles} alt={`Candles reflected in dark glass. The Circle at ${BRAND_AKAN}.`} h={220} aspect="21/9" /></Rv></Mx>
      </section>
      ...
      <section style={{ background: T.bg2, padding: "2px clamp(40px,6vw,80px)" }}>
        <Mx w={960}><Rv>
          <RealImg src={IMG.dinner} h={240} alt={`A composed room at ${BRAND_AKAN} before the guests arrive.`} aspect="21/9" />
        </Rv></Mx>
      </section>
                ...
                <div key={slide} aria-live="polite" style={{ background: c.lBg, position: "relative", overflow: "hidden", animation: "fadeUp .6s ease both" }}>
                  ...
                  <div style={{ padding: "clamp(32px,4vw,48px)" }}>
                    ...
                    <div style={{ borderLeft: `2px solid ${c.acc}30`, paddingLeft: 20, marginBottom: 24 }}>
                      <blockquote style={{ fontFamily: F.display, fontSize: "clamp(16px,1.8vw,20px)", fontWeight: 400, fontStyle: "italic", color: c.txt, lineHeight: 1.6 }}>{p.quote}</blockquote>
                    </div>
                    ...
                    <p style={{ fontFamily: F.body, fontSize: 12, fontWeight: 300, lineHeight: 1.8, color: c.sub, opacity: c.lBg === T.offW ? 1 : .6 }}>{p.interests}</p>
                  </div>
                </div>
