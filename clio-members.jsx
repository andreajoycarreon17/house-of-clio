...
export default function ClioMembers() {
  ...
  const EvCard = ({ e, mini }) => {
    const d = dk(e.bg); const isFull = e.st === "full"; const isBooked = booked.includes(e.id); const pHere = P.filter(m => m.next).length; return (
  ...
  /* ═══ EVENT DETAIL ═══ */
  if (pg === "event" && ve) {
    const e = ve; const d = dk(e.bg); const pInRoom = P.filter(m => m.next); return (
